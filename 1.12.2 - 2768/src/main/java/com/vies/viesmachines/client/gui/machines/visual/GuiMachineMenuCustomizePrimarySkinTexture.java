...
public class GuiMachineMenuCustomizePrimarySkinTexture extends GuiContainerVC {
    ...
    protected void actionPerformed(GuiButton parButton) 
    {
	    ...
	    {
			NetworkHandler.sendToServer(new MessageGuiMachineMenuCustomizePrimarySkinTextureHolidayEaster());
	    }
		if (parButton.id == 1104)
	    {
			NetworkHandler.sendToServer(new MessageGuiMachineMenuCustomizePrimarySkinTextureHoliday4thofJuly());
	    }
		if (parButton.id == 1105)
	    {
			NetworkHandler.sendToServer(new MessageGuiMachineMenuCustomizePrimarySkinTextureHolidayHalloween());
	    }
		if (parButton.id == 1106)
	    {
			NetworkHandler.sendToServer(new MessageGuiMachineMenuCustomizePrimarySkinTextureHolidayThanksgiving());
	    }
		if (parButton.id == 1107)
	    {
			NetworkHandler.sendToServer(new MessageGuiMachineMenuCustomizePrimarySkinTextureHolidayChristmas());
	    }
